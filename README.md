...
- `post1/` through `post9/` — standalone article pages
...
Run `node scripts/rebuild-articles.mjs` after changing article-shell metadata or shared navigation for posts 1–8. Post 9 has a standalone shell with article-specific graphics and evidence styling; maintain its metadata and navigation in `post9/index.html`. Article prose remains between the generated content markers in each post.
